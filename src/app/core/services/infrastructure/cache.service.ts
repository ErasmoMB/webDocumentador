...
export class CacheService {
  ...
  set(key: string, data: any, ttlMs?: number): void {
    try {
      ...
      this.storage.setItem(this.getCacheKey(key), JSON.stringify(entry));
    } catch (error) {
    }
  }
  ...
  get(key: string): any | null {
    try {
      const stored = this.storage.getItem(this.getCacheKey(key));
  ...
  saveResponse(url: string, params: any, data: any): void {
    try {
      ...
      this.storage.setItem(this.CACHE_PREFIX + cacheKey, JSON.stringify(entry));
  ...
  private saveMockDataByEndpoint(url: string, params: any, data: any): void {
    try {
      ...
      this.storage.setItem(`${this.MOCK_DATA_PREFIX}${endpointType}_${endpointKey}`, JSON.stringify(mockDataEntry));
  ...
  private updateConsolidatedMockData(endpointType: string, data: any): void {
    try {
      ...
      if (endpointType === 'poblacion' && data && data.data) {
        ...
        this.storage.setItem(consolidatedKey, JSON.stringify(data.data));
      }
  ...
  exportMockDataToJSON(): void {
    try {
      ...
      const poblacionData = this.storage.getItem(`${this.MOCK_DATA_PREFIX}consolidated_poblacion`);
      const peaData = this.storage.getItem(`${this.MOCK_DATA_PREFIX}consolidated_pea`);
      const poblacionDistritoData = this.storage.getItem(`${this.MOCK_DATA_PREFIX}consolidated_poblacion_distrito`);
  ...
  getCachedResponse(url: string, params: any): any | null {
    try {
      const cacheKey = this.generateCacheKey(url, params);
      const cached = this.storage.getItem(this.CACHE_PREFIX + cacheKey);
      ...
      if (this.isExpired(entry.timestamp)) {
        this.storage.removeItem(this.CACHE_PREFIX + cacheKey);
        return null;
      }
    ...
    try {
      ...
      keys.forEach(key => {
        if (key.startsWith(this.CACHE_PREFIX)) {
          const entry: CacheEntry = JSON.parse(this.storage.getItem(key) || '{}');
  ...
  importCacheFromJSON(jsonData: any): void {
    try {
      Object.keys(jsonData).forEach(key => {
        ...
        this.storage.setItem(this.CACHE_PREFIX + cacheKey, JSON.stringify(cacheEntry));
      });
