  ...
  private createStandardTransform(metadata: TableMetadata): (data: any) => any {
    ...
    return (data: any) => {
      ...
      return datosArray.map((item: any) => {
        ...
        if (customFields) {
          Object.entries(customFields).forEach(([backendField, frontendField]) => {
            transformed[frontendField] = this.getFieldValue(item, backendField);
          });
        }
