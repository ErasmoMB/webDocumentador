...
export class Seccion12FormComponent extends BaseSectionComponent implements OnDestroy {
  ...
  // ✅ OPTIMIZACIÓN: Una sola señal computada que obtiene TODOS los datos de una vez
  readonly allSectionData: Signal<Record<string, any>> = computed(() => {
    const allFields = [
      // Párrafos
      'parrafoSeccion12_salud_completo', 'parrafoSeccion12_educacion_completo', 'textoInfraestructuraEducacionPost',
      'parrafoSeccion12_recreacion_completo', 'parrafoSeccion12_deporte_completo',
      // Geo info
      'geoInfo',
      // Tablas
      'caracteristicasSaludTabla', 'cantidadEstudiantesEducacionTabla', 'ieAyrocaTabla', 'ie40270Tabla',
      'alumnosIEAyrocaTabla', 'alumnosIE40270Tabla',
      // Títulos y fuentes
      'caracteristicasSaludTitulo', 'caracteristicasSaludFuente',
      'cantidadEstudiantesEducacionTitulo', 'cantidadEstudiantesEducacionFuente',
      'ieAyrocaTitulo', 'ieAyrocaFuente', 'ie40270Titulo', 'ie40270Fuente',
      'alumnosIEAyrocaTitulo', 'alumnosIEAyrocaFuente', 'alumnosIE40270Titulo', 'alumnosIE40270Fuente'
    ];

    // Fotos: agregar todos los campos de fotos (5 prefijos x 10 fotos x 3 campos = 150 campos)
    ...
    photoPrefixes.forEach(prefix => {
      for (let i = 1; i <= 10; i++) {
        allFields.push(`${prefix}${i}Titulo`, `${prefix}${i}Fuente`, `${prefix}${i}Imagen`);
      }
    });
  ...
  readonly parrafoSaludSignal: Signal<string> = computed(() => {
    ...
    const manual = data['parrafoSeccion12_salud_completo'];
  ...
  readonly parrafoEducacionSignal: Signal<string> = computed(() => {
    const data = this.allSectionData();
    const manual = data['parrafoSeccion12_educacion_completo'];
    if (manual && String(manual).trim().length > 0) return String(manual);
    const grupoAISD = this.grupoAISDSignal();
  ...
  readonly parrafoInfraestructuraPostSignal: Signal<string> = computed(() => {
    const data = this.allSectionData();
    const manual = data['textoInfraestructuraEducacionPost'];
  ...
  readonly parrafoRecreacionSignal: Signal<string> = computed(() => {
    const data = this.allSectionData();
    const manual = data['parrafoSeccion12_recreacion_completo'];
    if (manual && String(manual).trim().length > 0) return String(manual);
    const grupoAISD = this.grupoAISDSignal();
  ...
  readonly parrafoDeporteSignal: Signal<string> = computed(() => {
    const data = this.allSectionData();
    const manual = data['parrafoSeccion12_deporte_completo'];
    if (manual && String(manual).trim().length > 0) return String(manual);
    const grupoAISD = this.grupoAISDSignal();
  ...
  readonly tablaSaludSignal: Signal<any[]> = computed(() => {
    const data = this.allSectionData();
    const v = data['caracteristicasSaludTabla'] ?? this.projectFacade.selectTableData(this.seccionId, null, 'caracteristicasSaludTabla')();
  ...
  readonly tablaEstudiantesSignal: Signal<any[]> = computed(() => {
    const data = this.allSectionData();
    const v = data['cantidadEstudiantesEducacionTabla'] ?? this.projectFacade.selectTableData(this.seccionId, null, 'cantidadEstudiantesEducacionTabla')();
  ...
  readonly tablaIEAyrocaSignal: Signal<any[]> = computed(() => {
    const data = this.allSectionData();
    const v = data['ieAyrocaTabla'] ?? this.projectFacade.selectTableData(this.seccionId, null, 'ieAyrocaTabla')();
  ...
  readonly tablaIE40270Signal: Signal<any[]> = computed(() => {
    const data = this.allSectionData();
    const v = data['ie40270Tabla'] ?? this.projectFacade.selectTableData(this.seccionId, null, 'ie40270Tabla')();
  ...
  readonly tablaAlumnosIEAyrocaSignal: Signal<any[]> = computed(() => {
    const data = this.allSectionData();
    const fromField = data['alumnosIEAyrocaTabla'];
    const fromTableData = this.projectFacade.selectTableData(this.seccionId, null, 'alumnosIEAyrocaTabla')();
  ...
  readonly tablaAlumnosIE40270Signal: Signal<any[]> = computed(() => {
    const data = this.allSectionData();
    const fromField = data['alumnosIE40270Tabla'];
    const fromTableData = this.projectFacade.selectTableData(this.seccionId, null, 'alumnosIE40270Tabla')();
    const resultado = fromField ?? fromTableData ?? [];
  ...
  readonly fotosSaludSignal: Signal<FotoItem[]> = computed(() => {
    ...
    for (let i = 1; i <= 10; i++) {
      const titulo = data[`${this.PHOTO_PREFIX_SALUD}${i}Titulo`];
      const fuente = data[`${this.PHOTO_PREFIX_SALUD}${i}Fuente`];
      const imagen = data[`${this.PHOTO_PREFIX_SALUD}${i}Imagen`];
      if (imagen) {
        fotos.push({
  ...
  readonly fotosIEAyrocaSignal: Signal<FotoItem[]> = computed(() => {
    ...
    const fotos: FotoItem[] = [];
    for (let i = 1; i <= 10; i++) {
      const titulo = data[`${this.PHOTO_PREFIX_IE_AYROCA}${i}Titulo`];
      const fuente = data[`${this.PHOTO_PREFIX_IE_AYROCA}${i}Fuente`];
      const imagen = data[`${this.PHOTO_PREFIX_IE_AYROCA}${i}Imagen`];
      if (imagen) {
        fotos.push({
          titulo: titulo || `Fotografía ${i}`,
          fuente: fuente || 'GEADES, 2024',
  ...
  readonly fotosIE40270Signal: Signal<FotoItem[]> = computed(() => {
    ...
    const fotos: FotoItem[] = [];
    for (let i = 1; i <= 10; i++) {
      const titulo = data[`${this.PHOTO_PREFIX_IE_40270}${i}Titulo`];
      const fuente = data[`${this.PHOTO_PREFIX_IE_40270}${i}Fuente`];
      const imagen = data[`${this.PHOTO_PREFIX_IE_40270}${i}Imagen`];
      if (imagen) {
        fotos.push({
          titulo: titulo || `Fotografía ${i}`,
          fuente: fuente || 'GEADES, 2024',
  ...
  readonly fotosRecreacionSignal: Signal<FotoItem[]> = computed(() => {
    ...
    const fotos: FotoItem[] = [];
    for (let i = 1; i <= 10; i++) {
      const titulo = data[`${this.PHOTO_PREFIX_RECREACION}${i}Titulo`];
      const fuente = data[`${this.PHOTO_PREFIX_RECREACION}${i}Fuente`];
      const imagen = data[`${this.PHOTO_PREFIX_RECREACION}${i}Imagen`];
      if (imagen) {
        fotos.push({
          titulo: titulo || `Fotografía ${i}`,
          fuente: fuente || 'GEADES, 2024',
  ...
  readonly fotosDeporteSignal: Signal<FotoItem[]> = computed(() => {
    ...
    const fotos: FotoItem[] = [];
    for (let i = 1; i <= 10; i++) {
      const titulo = data[`${this.PHOTO_PREFIX_DEPORTE}${i}Titulo`];
      const fuente = data[`${this.PHOTO_PREFIX_DEPORTE}${i}Fuente`];
      const imagen = data[`${this.PHOTO_PREFIX_DEPORTE}${i}Imagen`];
      if (imagen) {
        fotos.push({
          titulo: titulo || `Fotografía ${i}`,
          fuente: fuente || 'GEADES, 2024',
  ...
  readonly tituloCaracteristicasSaludSignal: Signal<string> = computed(() => {
    const data = this.allSectionData();
    const v = data['caracteristicasSaludTitulo'];
  ...
  readonly fuenteAlumnosIEAyrocaSignal: Signal<string> = computed(() => {
    const data = this.allSectionData();
    const v = data['alumnosIEAyrocaFuente'];
    return (v && String(v).trim().length > 0) ? String(v) : 'GEADES (2024)';
  });

  readonly tituloAlumnosIE40270Signal: Signal<string> = computed(() => {
    const data = this.allSectionData();
    const v = data['alumnosIE40270Titulo'];
  ...
  readonly fuenteAlumnosIE40270Signal: Signal<string> = computed(() => {
    const data = this.allSectionData();
    const v = data['alumnosIE40270Fuente'];
    return (v && String(v).trim().length > 0) ? String(v) : 'GEADES (2024)';
  });
  ...
  readonly photoFieldsHash: Signal<string> = computed(() => {
    ...
    for (const prefix of prefixes) {
      for (let i = 1; i <= 10; i++) {
        const titulo = data[`${prefix}${i}Titulo`] || '';
        const fuente = data[`${prefix}${i}Fuente`] || '';
        const imagen = data[`${prefix}${i}Imagen`] ? '1' : '0';
  ...
  onCaracteristicasSaludTableUpdated(updatedData?: any[]): void {
    const tablaDelState = this.projectFacade.selectTableData(this.seccionId, null, 'caracteristicasSaludTabla')();
    const datos = tablaDelState || updatedData || [];
    this.datos['caracteristicasSaludTabla'] = datos;
    this.onFieldChange('caracteristicasSaludTabla', datos, { refresh: true });
    this.cdRef.detectChanges(); // ✅ INMEDIATO
  }

  onCantidadEstudiantesTableUpdated(updatedData?: any[]): void {
    const tablaDelState = this.projectFacade.selectTableData(this.seccionId, null, 'cantidadEstudiantesEducacionTabla')();
    const datos = tablaDelState || updatedData || [];
    this.datos['cantidadEstudiantesEducacionTabla'] = datos;
    this.onFieldChange('cantidadEstudiantesEducacionTabla', datos, { refresh: true });
    this.cdRef.detectChanges(); // ✅ INMEDIATO
  }

  onIEAyrocaTableUpdated(updatedData?: any[]): void {
    const tablaDelState = this.projectFacade.selectTableData(this.seccionId, null, 'ieAyrocaTabla')();
    const datos = tablaDelState || updatedData || [];
    this.datos['ieAyrocaTabla'] = datos;
    this.onFieldChange('ieAyrocaTabla', datos, { refresh: true });
    this.cdRef.detectChanges(); // ✅ INMEDIATO
  }

  onIE40270TableUpdated(updatedData?: any[]): void {
    const tablaDelState = this.projectFacade.selectTableData(this.seccionId, null, 'ie40270Tabla')();
    const datos = tablaDelState || updatedData || [];
    this.datos['ie40270Tabla'] = datos;
    this.onFieldChange('ie40270Tabla', datos, { refresh: true });
    this.cdRef.detectChanges(); // ✅ INMEDIATO
  }

  onAlumnosIEAyrocaTableUpdated(updatedData?: any[]): void {
    const tablaDelState = this.projectFacade.selectTableData(this.seccionId, null, 'alumnosIEAyrocaTabla')();
    const datos = tablaDelState || updatedData || [];
    this.datos['alumnosIEAyrocaTabla'] = datos;
    this.onFieldChange('alumnosIEAyrocaTabla', datos, { refresh: true });
    this.cdRef.detectChanges(); // ✅ INMEDIATO
  }

  onAlumnosIE40270TableUpdated(updatedData?: any[]): void {
    const tablaDelState = this.projectFacade.selectTableData(this.seccionId, null, 'alumnosIE40270Tabla')();
    const datos = tablaDelState || updatedData || [];
    this.datos['alumnosIE40270Tabla'] = datos;
    this.onFieldChange('alumnosIE40270Tabla', datos, { refresh: true });
    this.cdRef.detectChanges(); // ✅ INMEDIATO
  }
  ...
  actualizarAlumnoAyroca(index: number, field: string, event: Event): void {
    ...
    if (index >= 0 && index < tablaActual.length) {
      ...
      this.onFieldChange('alumnosIEAyrocaTabla', tablaActualizada, { refresh: true });
      this.cdRef.markForCheck();
    }
  }
  ...
  actualizarAlumno40270(index: number, field: string, event: Event): void {
    ...
    if (index >= 0 && index < tablaActual.length) {
      ...
      this.onFieldChange('alumnosIE40270Tabla', tablaActualizada, { refresh: true });
      this.cdRef.markForCheck();
    }
  }
